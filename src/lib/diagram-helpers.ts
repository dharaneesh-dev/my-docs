...
/** A small set of labeled toggle/radio buttons appended below an SVG diagram. */
export function toggleGroup(
  ...
  const wrap = document.createElement("div");
  wrap.className = "mt-3 flex flex-wrap justify-center gap-1.5";
  let current = initial;
  ...
  const paint = () => {
    buttons.forEach((btn, value) => {
      btn.className =
        value === current
          ? "rounded-full border border-primary bg-primary/10 px-3 py-1 text-[12px] font-medium text-primary transition-colors"
          : "rounded-full border border-border bg-muted/40 px-3 py-1 text-[12px] text-muted-foreground transition-colors hover:text-foreground";
    });
  };
